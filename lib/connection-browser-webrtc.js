...
Connection.prototype.asyncHandleData = function (data) {
  return asink(function *() {
    ...
    let msg
    try {
      msg = DMsg().fromBuffer(buf) // Will throw if can't be parsed
      yield msg.asyncValidate() // Will throw if invalid
    } catch (error) {
      let error2 = new Error('Could not parse message from peer: ' + error.message)
...
Connection.prototype.sendDMsg = function (msg) {
