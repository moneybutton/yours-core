...
BIP44Account.prototype.toJSON = function () {
  ...
  json.bip32 = this.bip32.toFastHex()
  ...
  this.pathmap.forEach((keys, path) => {
    ...
    if (this.isPrivate()) {
      json.pathmap[path].xprv = keys.xprv.toFastHex()
    }

    json.pathmap[path].xpub = keys.xpub.toFastHex()
...
BIP44Account.prototype.fromJSON = function (json) {
  ...
  this.bip32 = BIP32().fromFastHex(json.bip32)
  ...
  Object.keys(json.pathmap).forEach((path) => {
    ...
    if (keys.xprv) {
      xprv = BIP32().fromFastHex(keys.xprv)
    }

    let xpub = BIP32().fromFastHex(keys.xpub)
