 ...
 * user.
 */
CoreUser.prototype.init = function () {
  this.user = User()
  this.dbuser = DBUser(this.db, this.user)
  let newuser = false
  return this.dbuser.get().then(user => {
    return user
  }).catch(err => {
    if (err) {
    ...
    newuser = true
    return this.user.asyncFromRandom()
  }).then(user => {
    this.user = user
    return user
  }).then(user => {
    if (newuser) {
      return this.dbuser.save(this.user)
    }
  }).then(() => {
    return this
  })
...
CoreUser.prototype.asyncSetName = function (name) {
  ...
  return this.dbuser.save(this.user).then(dbuser => {
    return this
  })
}
