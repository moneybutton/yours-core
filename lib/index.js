...
DattCore.prototype.asyncInitialize = function () {
  return spawn(function *() {
    ...
    this.corepeers = CorePeers(this.db)
    ...
    // established before considering the app to be "initialized". It should be
    // possible to run the app with no network connections. Obviously not
    // everything will work in that case, but the user should still be able to
    // access and modify their own data. Nonetheless, we do still want to kick
    // off the network connection initialization here - we just don't want to
    // wait for it to finish before resolving.
    this.corepeers.asyncInitialize().catch((err) => {
      if (err) {
