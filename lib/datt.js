...
Datt.prototype.init = function begin () {
  var self = this
...
Datt.prototype.getContent = function getContent (hash) {
  ...
  var contentPromise = self.getLocalContent(hash).then(function(localContent){
    deferred.resolve(localContent)
  }).fail(function() {
    self.contentListener.once('Received:'+hash, function(remoteContent) {
      console.log("Received the remote content!")
  ...
  switch (message.type) {
    ...
    case Message.Type.CONTENT:
      ...
      Content.fromObject(message.body.content).then(function(content) {
        ...
        console.log("Adding content to local storage")
        self.addContent(content)
        self.contentListener.emit('Received:'+hash, content)
      })
      break
    default:
