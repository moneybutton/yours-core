...
Content.prototype.init = function () {
  ...
  }.bind(this)).then(function (hashbuf) {
    if (hashbuf) {
...
Content.fromDataAndUser = function fromDataAndUser (data, user, post_time, post_height) {
  return user.then(function () {
    logger.debug('Content@fromDataAndUser: user ready')
    ...
    return content.init().then(function () {
      logger.debug('Content@fromDataAndUser: content ready')
      return content
    })
  })
}
...
Content.prototype.setOwnerPubKey = q.promised(function setOwnerPubKey (pubkey) {
  logger.debug('Content#setOwnerPubKey: ' + pubkey)
  logger.debug(this instanceof Content)
  if (!pubkey) {
    return null
  }
  ...
  logger.debug("Content#setOwnerPubKey: set owner pub key = '" + pubkey + "'")
...
Content.prototype.setOwnerAddress = q.promised(function setOwnerAddress (address) {
  logger.debug('Content#setOwnerAddress: ' + address)
...
Content.prototype.setSignature = function setSignature (newSig) {
  newSig = q(newSig)
  logger.debug('Content#setSignature')
  logger.debug(JSON.stringify(newSig, null, 4))
  return newSig.then(function (sig) {
    if (!sig) {
      return null
    }
    logger.debug("Content#setSignature - signature ready: '" + sig + "'")
    logger.debug(JSON.stringify(sig, null, 4))
    ...
    return p_sigvalid.then(function (sigvalid) {
      ...
      logger.debug("Content#setSignature: set '" + this.signature + "'")
...
Content.fromObjects = q.promised(function fromObjects (objects) {
  return q.all(u.map(objects, Content.fromObject))
})
