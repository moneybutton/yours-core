 ...
 */
ContentDiscovery.prototype.handleContentDiscoveryRequest = function handleContentDiscoveryRequest (message, main) {
  var sender = message.body.sender
  var hash = message.body.hash

  // Won't forward my own messages
  if (sender === main.peer.id) {
  ...
  var broadcast = main.broadcastMessage(JSON.stringify(message))

  var getAndAnnounce = main.getContent(hash).then(
    // Content found
    function (content) {
      ...
      // answer requester
      var peers = []
      peers.push(main.peer.id)
      var announceMessage = Message.announcePeersForHash(hash, peers)
      return main.sendMessage(announceMessage.serialize(), sender)
    },
    // Content not found
    function () {
 ...
 * that have a hash.
 */
ContentDiscovery.prototype.findPeersForContent = function findPeersForContent (hash, main) {
  var message = Message.requestPeersForHash(hash, main.peer.id)
  logger.debug('Broadcasting')
  main.broadcastMessage(message.serialize())
 ...
 */
ContentDiscovery.prototype.handleAnnouncePeersForHash = function handleAnnouncePeersForHash (message, main) {
  var hash = message.body.hash
  ...
  for (var i in peers) {
    ...
    var requestMessage = Message.contentRequestByHash(hash, main.peer.id)
    logger.debug('sending requestMessage ' + requestMessage.serialize() + ' for ' + hash + ' to ' + peer)
    main.sendMessage(requestMessage.serialize(), peer)
 ...
 */
ContentDiscovery.prototype.findPeersForContent = function findPeersForContent (hash, main) {
  var message = Message.requestPeersForHash(hash, main)
  logger.debug('Broadcasting')
  main.broadcastMessage(message.serialize())
  this.contentRequested[hash] = 1
}
