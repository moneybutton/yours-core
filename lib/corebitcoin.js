...
CoreBitcoin.prototype.asyncInitialize = function (user) {
  return asink(function *() {
    try {
      this.dbbip44wallet = DBBIP44Wallet(this.db)
      this.bip44wallet = yield this.dbbip44wallet.asyncGet()
    } catch (err) {
...
CoreBitcoin.prototype.asyncGetAddressesBalancesSatoshis = function (addresses) {
  return this.blockchainAPI.asyncGetAddressesBalancesSatoshis(addresses)
}

CoreBitcoin.prototype.asyncGetAddressesIndividualBalancesSatoshis = function (addresses) {
  return this.blockchainAPI.asyncGetAddressesIndividualBalancesSatoshis(addresses)
}

CoreBitcoin.prototype.asyncBuildTransaction = function (paymentDescriptions) {
  return asink(function *() {
    ...
    utxos.forEach((obj) => {
      txb.fromPubkeyhash(obj.txhashbuf, obj.txoutnum, obj.txout, obj.pubkey)
    })
    for (let i = 0; i < paymentDescriptions.length; i++) {
      let toAddress = paymentDescriptions[i].toAddress
      let toAmountSatoshis = paymentDescriptions[i].toAmountSatoshis
      txb.toAddress(BN(toAmountSatoshis), toAddress)
    }
    txb.build()
    return txb
  }, this)
...
CoreBitcoin.prototype.asyncBuildSignAndSendTransaction = function (paymentDescriptions) {
  return asink(function *() {
    let txb
    txb = yield this.asyncBuildTransaction(paymentDescriptions)
...
CoreBitcoin.prototype.asyncGetExtAddress = function (index) {
  return asink(function *() {
    // Every time the wallet is changed, we must check to see if it has already
    // been saved by checking if the revision has changed (for instance, the
    // user has created a new address in a different tab). If so, we need to
    // load that wallet first (containing the new addresses) before performing
    // actions on it. TODO: It shouldn't be necessary to reload the entire
    // wallet - just reload the newly created addresses.
    if (yield this.dbbip44wallet.asyncRevHasChanged()) {
      this.bip44wallet = yield this.dbbip44wallet.asyncGet()
    }
