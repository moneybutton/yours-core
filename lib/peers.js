 ...
 * code should run both in node and a browser. This code is basically an
 * abstraction over the different types of network interfaces we support, and
 * is necessary because, unlike most apps, we support Web RTC and Web Sockets
...
Peers.prototype.monitorNetworkWebRTC = function () {
  ...
  this.networkWebRTC.on('dmsg', this.handleNetworkWebRTCDMsg.bind(this))
  return this
}
...
Peers.prototype.handleNetworkWebRTCDMsg = function (obj) {
  this.emit('dmsg', {
    network: this.networkWebRTC,
...
Peers.prototype.broadcastDMsg = function (msg) {
  // TODO: Work for web sockets also
  if (this.networkWebRTC) {
    this.networkWebRTC.connections.forEach((connection) => {
      connection.sendDMsg(msg)
    })
  }
  return this
}
