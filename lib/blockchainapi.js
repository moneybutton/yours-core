    ...
    let res = yield new Promise((resolve, reject) => {
      request(this.blockchainAPIURI + urlquery, function (error, response, body) {
        if (error || response.statusCode !== 200) {
          error = new Error(`blockchainapi getting ${urlquery}: ${error} - statusCode: ${response.statusCode}`)
  ...
  return asink(function *() { //eslint-disable-line
    ...
    let res = yield new Promise((resolve, reject) => {
      ...
      request.post(options, function (error, response, body) {
        if (error || response.statusCode !== 200) {
          error = new Error(`blockchainapi posting ${urlquery}: ${error} - statusCode: ${response.statusCode}`)
          error.statusCode = response.statusCode
          error.body = body
          return reject(error)
        }
