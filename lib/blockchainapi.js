 ...
 * Note that this gets all UTXOs, including those with no confirmations.
 */
BlockchainAPI.prototype.asyncGetPayerAddresses = function (address) {
  return asink(function *() {
    let addressString = yield address.asyncToString()
    let TXsJSON = yield this.asyncGetRequest('txs/?address=' + addressString)
    let addresses = []

    for (let i = 0; i < TXsJSON.txs.length; i++) {
      // sometimes the api returns an undefined scriptSig
      if (TXsJSON.txs[i].vin[0].scriptSig !== undefined) {
        let scriptSigHex = TXsJSON.txs[i].vin[0].scriptSig.hex
