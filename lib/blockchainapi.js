...
BlockchainAPI.prototype.asyncGetBlockchainPayerAddresses = function (address) {
  return asink(function *() {
    ...
    for (let i = 0; i < TXsJSON.items.length; i++) {
      ...
      // TODO: Replace Insight with a reliable blockchain explorer so we don't
      // have to account for weird edge cases like this.
      ...
      } else if (TXsJSON.items[i].vin[0].addr !== undefined) {
        let address = yield Address().asyncFromString(TXsJSON.items[i].vin[0].addr)
        addresses.push(address)
      }
    }
...
/**
 * Get balances for each address in addresses. Does not sum the total like
 * asyncGetAddressesBalancesSatoshis - rather, it returns a list of objects,
 * one for each address, that contains its confirmed balance, unconfirmed
 * balance, and total balance.
 */
BlockchainAPI.prototype.asyncGetAddressesIndividualBalancesSatoshis = function (addresses) {
  return asink(function *() {
    let json = yield this.asyncGetUTXOsJSON(addresses)
    let balancesmap = {}
    json.forEach((obj) => {
      let addressstr = obj.address
      if (!balancesmap[addressstr]) {
        balancesmap[addressstr] = {
      ...
      if (obj.confirmations > 0) {
        balancesmap[addressstr].confirmedBalanceSatoshis += obj.amount * 1e8
      } else {
        balancesmap[addressstr].unconfirmedBalanceSatoshis += obj.amount * 1e8
      }
      balancesmap[addressstr].totalBalanceSatoshis += obj.amount * 1e8
    })
    let balances = []
    for (let index = 0; index < addresses.length; index++) {
      let addressstr = yield addresses[index].asyncToString()
      if (balancesmap[addressstr]) {
        balances[index] = balancesmap[addressstr]
      } else {
        balances[index] = {
          confirmedBalanceSatoshis: 0,
          unconfirmedBalanceSatoshis: 0,
