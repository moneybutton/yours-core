 ...
 * ============================
 *
 * A message on the datt p2p network. It is designed to be similar to, but not
 * the same as, a bitcoin p2p message.
 */
"use strict";
let dependencies = {
  ...
  Constants: require('fullnode/lib/constants').Default,
...
function inject(deps) {
  ...
  Msg.prototype.fromPing = function(obj) {
    this.fromObject(obj);
    if (!this.datacmd)
      this.setCmd('ping');
    if (!this.databuf)
  ...
  Msg.prototype.fromPong = function(obj) {
    this.fromObject(obj);
    if (!this.datacmd)
      this.setCmd('pong');
    if (!this.databuf)
  ...
  Msg.prototype.fromPongFromPing = function(pingmsg) {
    this.fromPing(pingmsg);
    delete this.cmdbuf;
    this.fromPong(pingmsg);
    return this;
  };
...
let Msg = inject();
Msg.Mainnet = inject({
  Constants: require('fullnode/lib/constants').Mainnet
});
Msg.Testnet = inject({
  Constants: require('fullnode/lib/constants').Testnet
});
module.exports = Msg;
