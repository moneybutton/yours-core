 ...
 * messages".
 */
function Main (coordinationServerConfig, peer, peers, user, seenMessages, contentRequested) {
  var config = u.extend({
    debug: 3,
  ...
  this.peer = peer || new Peer(config)
 ...
 */
Main.prototype._connectToAvailablePeers = function _refreshConnections () {
  this.connections = this.connections || {}
...
Main.prototype.handleContentDiscoveryRequest = function handleContentDiscoveryRequest (message) {
  ...
  var getAndAnnounce = this.getContent(hash).then(
    // Content found
    function (content) {
 ...
 */
Main.prototype.findPeersForContent = function findPeersForContent (hash) {
  var message = Message.requestPeersForHash(hash)
  logger.debug('Broadcasting')
  this.broadcastMessage(message.serialize())
  this.contentRequested[hash] = 1
}
