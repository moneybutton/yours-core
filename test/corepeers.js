...
describe('CorePeers', function () {
  ...
  describe('#handleDMsgPing', function () {
    it('should send a pong in response to a ping', function () {
      ...
      let connection = {}
      connection.sendDMsg = sinon.spy()
      let obj = {
        msg: msgPing.toDMsg(),
        connection: connection
      }
      let corepeers = CorePeers()
      corepeers.handleDMsgPing(obj)
      connection.sendDMsg.calledOnce.should.equal(true)
      connection.sendDMsg.getCall(0).args[0].toHex().should.equal(msgPong.toDMsg().toHex())
    })
  })

  describe('asyncHandleDMsgContentAuth', function () {
    it('should emit peers-contentauth on valid contentauth', function () {
      return asink(function *() {
        ...
        yield corepeers.asyncHandleDMsgContentAuth({msg})
    ...
    it('should be able to connect to another node and exchange ping/pong', function () {
      return asink(function *() {
        ...
        yield new Promise((resolve, reject) => {
          let msgPing = DMsgPing().fromRandom()
          corepeers1.on('dmsg', (obj) => {
            let msg = obj.msg
            msg.getCmd().should.equal('pong')
            resolve()
          })
          corepeers1.broadcastDMsg(msgPing.toDMsg())
        })
        corepeers1.close()
        corepeers2.close()
      })
