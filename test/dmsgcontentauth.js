...
describe('DMsgContentAuth', function () {
  ...
  it('should exist', function () {
    should.exist(DMsgContentAuth)
    should.exist(DMsgContentAuth())
    should.exist(DMsgContentAuth(contentauth))
  })

  describe('#toBuffer', function () {
    it('should be 32 bytes (one hash) longer than contentauth', function () {
      let msgcontentauth = DMsgContentAuth().fromContentAuth(contentauth)
  ...
  describe('#fromBuffer', function () {
    it('should convert from known buffer', function () {
      ...
      let msgcontentauth = DMsgContentAuth().fromBuffer(buf)
  ...
  describe('#asyncFromContentAuth', function () {
    it('should get a msgcontentauth from contentauth', function () {
      return asink(function *() {
        let msgcontentauth = yield DMsgContentAuth().asyncFromContentAuth(contentauth)
        msgcontentauth.hashbuf.toString('hex').should.equal(contentauth.getHash().toString('hex'))
      })
    })
  })
  ...
  describe('#fromDMsg', function () {
    it('should convert from a msg', function () {
      let msgcontentauth = DMsgContentAuth().fromContentAuth(contentauth)
      let msg = msgcontentauth.toDMsg()
      let msgcontentauth2 = DMsgContentAuth().fromDMsg(msg)
